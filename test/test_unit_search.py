...
if __name__ == '__main__':
    result = unittest.main(testRunner=HtmlTestRunner.HTMLTestRunner(output='./test-reports'), verbosity=2)

    # Imprimir el resumen de los resultados
    print("\nTest results:")
    print(result.result)  # Esto imprimirá los resultados de las pruebas en la consola

 # Verifica si el archivo HTML se generó
    html_report_path = os.path.join(result, "test_report.html")
    if os.path.exists(html_report_path):
        print(f"El archivo HTML se generó correctamente en: {html_report_path}")
    else:
        print("No se generó el archivo HTML.")
